...
# On definit une matrice d'adjacence du graphe initial
M_adj1 = [
...
def make_graph1(M):
	""" int matrix -> graph : crée un graphe a partir de la matrice d'adjacence en entier """
	n = len(M)
	res = initGraph(range(n),[])
	for i in range(1,n):
		for j in range(i):
			if M[i][j]> 0:
				addEdge(res,i,j)
...
def make_graph2(M):
	""" int matrix -> graph : crée un graphe a partir de la matrice d'adjacence en bool """
	n = len(M)
	res = initGraph(range(n),[])
	for i in range(1,n):
		for j in range(i):
...
G1 = make_graph1(M_adj1) # Le graphe initial

# On definit une matrice d'adjacence du graphe recouvrant minimal de type bool matrix
M_adj2 = [[False]*(len(M_adj1)) for i in range(len(M_adj1))] # Initialement vide


G2 = initGraph() # Graphe sans aretes!



def is_connex(G):
	""" graph -> bool : determine si le graphe est connexe """
	if vertices(G) == []:
		return True
	""" cas du graphe vide"""
	v1 = G["vertices"][0] # On part d'un sommet au hasard
	parcours = [] # liste contenant le parours en largeur des sommets
	F = emptyQueue() # parcours en largeur donc on utilise une pile
	# On realise un parcours en largeur et on verifie que l'on tous les sommets
	F = enqueue(F,v1)
	while (not isEmpty(F)): # On continue tant que la file n'est pas vide, ie qu'il n'y a plus de sommets a traiter
		v = first(F) # Le sommet a traiter
		F = dequeue(F)
		if not(v in parcours):
			parcours = parcours + [v] # on ajoute v au sommet parcourus
		voisins = successors(G,v) + predecessors(G,v)  # Liste des voisins de v, a ajouter en file
		for i in voisins:
			if not(i in parcours): # On verifie que l'on a pas deja traité ce sommet
				F = enqueue(F,i) # Alors on peut l'ajouter a la file 
	return(nbVertices(G) == len(parcours)) 

def is_connected(G,v1,v2):
	""" graph * int * int -> bool : determine si s1 est relié à s2 dans G """
	parcoursv1 = [] # liste contenant le parcours en largeur à partir de v1
	F = emptyQueue() # file du parcours en largeur
	F = enqueue(F,v1)
	while (not isEmpty(F)): # On continue tant que la file n'est pas vide, ie qu'il n'y a plus de sommets a traiter
		v = first(F) # Le sommet a traiter
		F = dequeue(F)
		if not(v in parcoursv1):
			parcoursv1 = parcoursv1 + [v] # on ajoute v au sommet parcourus
		voisins = successors(G,v) + predecessors(G,v)  # Liste des voisins de v, a ajouter en file
		for i in voisins:
			if not(i in parcoursv1): # On verifie que l'on a pas deja traité ce sommet
				F = enqueue(F,i) # Alors on peut l'ajouter a la file 	
	return v2 in parcoursv1
...
def find_min(M,l):
	""" matrix * int*int list -> int * int * int : renvoie l'arete dont le poids le plus petit parmi ceux n'appartenant pas a l """
	n = len(M)
	res = 10000
	sommet_i = 1
	sommet_j = 0
	for i in range(1,n):
		for j in range(i):
			if ((M[i][j] <= res) and (M[i][j] > 0) and not((i,j) in l)):
				res = M[i][j]
				sommet_i = i
				sommet_j = j
	return(res,sommet_i,sommet_j)
...
# On remplit M_adj2
def recouvrir(G):
	""" graph -> unit : remplit M_adj2 de sorte qu'il represente le graphe recouvrant minimal """
	current_edges = []
	while ( not( is_connex(G) and (nbVertices(G) == nbVertices(G1)) ) ): # On continue a recouvrir tant que G2 n'est pas connexe et n'a pas tous les sommets
		(a,s1,s2) = find_min(M_adj1,current_edges)
		current_edges = current_edges + [(s1,s2)]
		print(s1,s2)
		print G
		if not((s1 in vertices(G)) and (s2 in vertices(G)) and (is_connected(G,s1,s2))): 
			# Si l'un des deux sommets n'est pas connecté au graphe on relie
			# Ou si les sommets ne sont pas dans la meme composante connexe du graphe on relie de meme
			M_adj2[s1][s2] = True
			M_adj2[s2][s1] = True
			G = addEdge(G,s1,s2)
			G = addVertex(G,s1) # on ajoute les sommets
			G = addVertex(G,s2)
...
def euler_107():
	""" unit -> int : calcul le gain avec l'arbre recouvrant """
	S1 = 0 # Somme des poids des aretes de G1
	for i in range(1,len(M_adj1)):
		for j in range(i):
			S1+= M_adj1[i][j]
	print S1
	S2 = 0 # Somme des poids des aretes de G2
	for i in range(1,len(M_adj1)):
		for j in range(i):
			if M_adj2[i][j]:
				S2 += M_adj1[i][j] 
...
def main():
	print(M_adj1)
	print(M_adj2)
	print(make_graph1(M_adj1))
	printGraph(G1)
	print(is_connex(G1))
	printGraph(G2)
	print(find_min(M_adj1,[]))
	print(find_min(M_adj1,[]))
	recouvrir(G2)
	printGraph(G2)
	print(M_adj2)
